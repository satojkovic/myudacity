...
def main():
    ...
    batches, labels = generate_batch_cbow(batch_size=8,
                                          skip_window=1)
    for i in range(8):
        print('%d %d -> %d' % (batches[i, 0], batches[i, 1], labels[i, 0]))
        print('%s %s -> %s' % (reverse_dictionary[batches[i, 0]],
                               reverse_dictionary[batches[i, 1]],
